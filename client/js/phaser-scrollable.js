...
ScrollableArea.prototype.moveCanvas = function (pointer, x, y) {
		...
		this.limitMovement();
		//NewsIndexScreen.prototype.GroupingDummyRectangle();
};
...
ScrollableArea.prototype.update = function () {
		...
		if (this.autoScrollY && this.amplitudeY != 0) {
				...
				if (delta > 0.5 || delta < -0.5) {
						...
						//currentScreenDummyRectangleGroup.position.y = this.targetY + delta;
						// CurrentScoreScreen.prototype.SetTheCurrentScoreButtonGroupScrollable(this.targetY + delta);
						// NewsIndexScreen.prototype.SetDummyRectangleGroupPos(this.targetY + delta);
						isScrollable = true;
				}
				else {
						this.autoScrollY = false;
		...
		if (this.settings.verticalWheel && this.velocityWheelYAbs > 0.1) {
				...
				// currentScreenDummyRectangleGroup.position.y += this.velocityWheelY;
				// CurrentScoreScreen.prototype.SetTheCurrentScoreButtonGroupScrollable(currentScreenDummyRectangleGroup.position.y  + this.velocityWheelY);
				// NewsIndexScreen.prototype.SetDummyRectangleGroupPos(dummyRectangleGroup.position.y  + this.velocityWheelY);
...
ScrollableArea.prototype.limitMovement = function() {
	// console.log("Enter into the limit Movement");
	if (this.settings.horizontalScroll) {
		if (this.x > this._x)
			this.x = this._x;
	...
	if (this.settings.verticalScroll) {
		if (this.y > this._y){
			this.y = this._y;
			// currentScreenDummyRectangleGroup.position.y = this._y;
			// game.world.bringToTop(currentScreenDummyRectangleGroup);
			//CurrentScoreScreen.prototype.SetTheCurrentScoreButtonGroupScrollable(this._y);
			//NewsIndexScreen.prototype.SetDummyRectangleGroupPos(this._y);
		}
		if (this.y < -(this.height-this._h-this._y)) {
			if (this.height > this._h) {
				this.y = -(this.height-this._h-this._y);
				// currentScreenDummyRectangleGroup.position.y =  -(this.height-this._h-this._y);
				// game.world.bringToTop(currentScreenDummyRectangleGroup);
				//CurrentScoreScreen.prototype.SetTheCurrentScoreButtonGroupScrollable(-(this.height-this._h-this._y));
				//NewsIndexScreen.prototype.SetDummyRectangleGroupPos(-(this.height-this._h-this._y));
			}
			else {
				this.y = this._y;
				// currentScreenDummyRectangleGroup.position.y = this._y;
